...
led_pin = 2
brillo = 0  # Valor inicial de brillo
...
led_pwm = PWM(Pin(led_pin), freq=1000, duty=brillo)

def pagina(brillo, estado_led, valorpot):
    html = """<!DOCTYPE html>
<html lang="en">
    ...
    <main>
        <a class="logotipo" id="logotipo">
            ...
            <div class="slidecontainer"> 
                <input class="slider" id="scroll" type="range" min="0" max="1023" value=""" + str(brillo) + """ onchange="cambiarBrillo(this.value)">                
            </div>
...
<p> <strong id="elvalor" class="button"> """+valorpot+ """</strong></p>



<a href="/on"> <button class="button" id="EncenderBoton" onclick="encenderApagar(1)">¡ On !</button></a>
<a href="/off"><button class="button" id="ApagarBoton" onclick="encenderApagar(0)">¡ Off !</button></a>
<br>
<br>
<br>
<br>
...
function cambiarBrillo(valor) {
            ...
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    document.getElementById("elvalor").innerHTML = this.responseText;
                }
            };
            xhttp.open("GET", "/set_brillo?valor=" + valor, true);
            xhttp.send();
        }

        function encenderApagar(estado) {
            var xhttp = new XMLHttpRequest();
            xhttp.open("GET", "/set_estado?estado=" + estado, true);
            xhttp.send();
        }
...
while True:
    valorpot= str(volume.read())

    conex, addr = elservidor.accept()
    request = conex.recv(1024)
    request = str(request)
    if request.find('/set_brillo?valor=') != -1:
        brillo = int(request.split('/set_brillo?valor=')[1].split(' ')[0])
        led_pwm.duty(brillo)  # Ajustar el brillo usando PWM
        
    if request.find('/set_estado?estado=') != -1:
        estado = int(request.split('/set_estado?estado=')[1].split(' ')[0])
        if estado == 1:
            led_pwm.duty(100)  # Encender el LED con el brillo actual
            estado_led = "Encendido"
        elif estado == 0:
            led_pwm.duty(0)  # Apagar el LED
            estado_led = "Apagado"
            
    Web = pagina(brillo, estado_led, valorpot)
